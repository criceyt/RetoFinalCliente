...
public class NavegacionPrincipalController implements Initializable {
    ...
    private ComboBox<String> comboBoxModelos = new ComboBox<>();// El StackPane que contiene la imagen y el contenido
    ...
    private void abrirVentanaInformacionVehiculo(ActionEvent event, Vehiculo vehiculo) {
        try {
            ...
            Logger.getLogger(InformacionExtraVehiculoController.class
                    .getName()).log(Level.SEVERE, null, ex);
            new Alert(Alert.AlertType.ERROR, "Error en la sincronización de ventanas, intentalo más tarde.", ButtonType.OK).showAndWait();
        }
    }
